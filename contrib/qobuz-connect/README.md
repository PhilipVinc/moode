...
```bash
sudo /home/moode/qobuz-gui-backup-*/restore.sh
sudo reboot
```

Every file the installer replaced was backed up there, and the restore is targeted: it
...
comes from, and what is deliberately left out. Four files are vendored from a 10.3.2 Pi
rather than taken from the branch; `payload-10.3.2/CAPTURED.md` says why, and `pack.sh`
warns when the branch moves under one of them.
